import time

import ollama
from fastapi import FastAPI

from src.config import LLM_MODEL, LLM_TEMPERATURE
from src.logging import setup_logging
from src.schemas import LLMRequest, LLMResponse

logger = setup_logging("llm")

app = FastAPI(title="LLM Service")


@app.post("/generate", response_model=LLMResponse)
def generate(req: LLMRequest) -> LLMResponse:
    temp = req.temperature or LLM_TEMPERATURE
    logger.info("generate model=%s temp=%.2f prompt=%s", LLM_MODEL, temp, req.prompt[:100])
    t0 = time.time()

    resp = ollama.generate(model=LLM_MODEL, prompt=req.prompt, options={"temperature": temp})

    dt = time.time() - t0
    tokens = resp.get("eval_count", 0)
    output = resp["response"]
    logger.info("done tokens=%d time=%.2fs output=%s", tokens, dt, output[:200])
    return LLMResponse(text=output, tokens=tokens, time_s=dt)


@app.get("/health")
def health():
    return {"status": "ok", "model": LLM_MODEL}
